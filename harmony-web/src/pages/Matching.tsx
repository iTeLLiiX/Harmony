...
import { HarmonyButton, LikeButton, SuperLikeButton, PassButton, MatchingButtonGroup } from '../components/HarmonyButton';
import { GlassmorphismCard } from '../components/Glassmorphism';
import {
  Close,
  Favorite,
  Star,
...
const Matching: React.FC = () => {
  ...
  const handleLike = async () => {
    ...
    // Paywall-Check für Likes
    const canLike = await checkFeature('unlimited_likes');
    if (!canLike) {
      showPaywall('unlimited_likes');
      return;
    }
  ...
  const handleSuperLike = async () => {
    ...
    // Paywall-Check für Super Likes
    const canSuperLike = await checkFeature('super_likes');
    if (!canSuperLike) {
      showPaywall('super_likes');
      return;
    }
